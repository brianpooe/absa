...
type ComponentState<Store extends ComponentStore<any>> =
  Store extends ComponentStore<infer State> ? State : any;
...
type ComponentStateSelectors<Store extends ComponentStore<any>> = {
  [K in ComponentStateKeys<Store>[number] as `${K}$`]: Observable<
    ComponentState<Store>[K]
  >;
};
